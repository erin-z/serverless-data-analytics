...
​	where username is your username (e.g user1) and click Finish.
...
2. To analyze all the taxi rides for January 2016, you start with a set of data in S3. First, create a database for this workshop within AWS Glue. A database is a set of associated table definitions, organized into a logical group. In Athena, database names are all lowercase, no matter what you type.
   ...
   iii. Enter the Database name as **username-glue**. You can skip the description and location fields and click on **Create**. 
...
3. Click on **Crawlers** under Data Catalog column on the left. 
   ...
   ​	a. For **Database**, select the database created earlier, **username-glue**.
...
3. Under Job properties, input name as **username-yellow**. Since we will be working with only the yellow dataset for this workshop.

   i. Under  IAM Role, Choose the IAM role created at the beginning of this lab (**userXX-glue-role**). 

   x. Under This job runs, choose the radio button for **A proposed script generated by AWS Glue**.

   xi. For Script file name, enter **username-yellow**.
...
5. Under Choose your data sources, select **reinv17_yellow** table in your database as the data source and click on **Next**.
...
2. Under Database, you should see the database **username-glue** which was created during the previous section. 
...
12. For Configure the crawler's output, Click **Add Database** and enter **username-glue-parquet** as the database name and click **create**. For Prefix added to tables, you can enter a prefix **parq_** and click **Next**.

13. Review the Crawler Info and click **Finish**. Click on **Run it Now?**. 

14. Click on **Tables** on the left, and for database username-glue-parquet you should see the table parq_target. Click on the table name and you will see the MetaData for this converted table. 
...
16. Under Database, you should see the database **username-glue-parquet** which was just created. Select this database and you should see under Tables **parq_target**.
...
18. Under Database, you should see the earlier made database **username-glue** which was created in a previous section. Select this database and you should see under Tables **reinv17_yellow**. 
